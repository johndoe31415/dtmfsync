 ...
 *
 *   Generated at: 2020-09-02 12:01:01
 */
...
bool argparse_parse(int argc, char **argv, argparse_callback_t argument_callback, argparse_plausibilization_callback_t plausibilization_callback) {
	...
	struct option long_options[] = {
		{ "filename",                         required_argument, 0, ARG_FILENAME_LONG },
		{ 0 }
	};
	...
	last_parsed_option = ARG_FILENAME;
	if (!argument_callback(ARG_FILENAME, argv[positional_index++], errmsg_callback)) {
		return false;
	}
...
void argparse_show_syntax(void) {
	fprintf(stderr, "usage: tsfind filename\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Find timestamps in audio or video data\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "positional arguments:\n");
	fprintf(stderr, "  filename  Audio or video data to find encoded scrambled timestamps in.\n");
}
	...
	if (!argparse_parse(argc, argv, argument_callback, plausibilization_callback)) {
		if (last_parsed_option > ARGPARSE_POSITIONAL_ARG) {
			if (last_error_message[0]) {
				fprintf(stderr, "tsfind: error parsing argument %s -- %s\n", option_texts[last_parsed_option], last_error_message);
			} else {
				fprintf(stderr, "tsfind: error parsing argument %s -- no details available\n", option_texts[last_parsed_option]);
			}
		} else if (last_parsed_option == ARGPARSE_POSITIONAL_ARG) {
			fprintf(stderr, "tsfind: error parsing optional arguments -- %s\n", last_error_message);
		}
...
static const char *option_enum_to_str(enum argparse_option_t option) {
	switch (option) {
		case ARG_FILENAME: return "ARG_FILENAME";
	}
	return "UNKNOWN";
}
